...
(function () {
  ...
  function show(el) {
    if ('none' === el.style.display)
      el.style.display =
        '' !== el.style.getPropertyValue('--touchTouchDisplay')
          ? el.style.getPropertyValue('--touchTouchDisplay')
          : 'block';
    return el;
  }
  ...
  function fit(img, scale) {
    ...
    var w = img.width,
      ...
      wh =
        scale * (window.innerHeight || document.documentElement.clientHeight);
    if ((h * ww) / w > wh) {
  ...
  function touchTouch(items, options) {
    ...
    removeExtraHandlers = function removeExtraHandlers() {
      ...
      if (touchEnd) {
        if (touchEnd.isTouch) {
          removeEvent(slider, 'touchend', touchEnd, {
            passive: true,
            capture: false,
          });
          removeEvent(slider, 'touchcancel', touchEnd, {
            passive: true,
            capture: false,
          });
        } else {
          removeEvent(slider, 'mouseup', touchEnd, {
            passive: true,
            capture: false,
          });
        }
    ...
    closeBtn = addClass(document.createElement('a'), 'tt-close-button');
      ...
      (touchStart = debounce(
        function (e) {
          ...
          if ('img' === el.tagName.toLowerCase()) {
            if (
              (isTouch && 2 === e.touches.length) ||
              (!isTouch && (e.ctrlKey || e.metaKey))
            ) {
              removeExtraHandlers();
              var touches = isTouch
              ...
              addEvent(
                ...
                (touchMove = function (e) {
                  ...
                  var touches2 = isTouch
                    ? [
                        touches[0].identifier === e.touches[0].identifier
                          ? e.touches[0]
                          : touches[0].identifier === e.touches[1].identifier
                          ? e.touches[1]
                          : null,
                        touches[1].identifier === e.touches[0].identifier
                          ? e.touches[0]
                          : touches[1].identifier === e.touches[1].identifier
                          ? e.touches[1]
                          : null,
                      ]
                    : null;
                  ...
                  if (isTouch) {
                    ...
                    a =
                      stdMath.hypot(end[2] - end[0], end[3] - end[1]) /
                        stdMath.hypot(
                          start[2] - start[0],
                          start[3] - start[1]
                        ) -
              ...
              if (isTouch) {
                addEvent(slider, 'touchend', touchEnd, {
                  passive: true,
                  capture: false,
                });
                addEvent(slider, 'touchcancel', touchEnd, {
                  passive: true,
                  capture: false,
                });
              } else {
                addEvent(slider, 'mouseup', touchEnd, {
                  passive: true,
                  capture: false,
                });
              }
              ...
              removeExtraHandlers();
              var touch = isTouch ? e.touches[0] : null,
                startX = isTouch ? touch.pageX : e.pageX;
              ...
              addEvent(
                ...
                (touchMove = function (e) {
                  ...
                  var diff = (isTouch ? e.touches[0].pageX : e.pageX) - startX,
                    threshold = isTouch ? move_m : move_d;
    ...
    addEvent(slider, 'mousedown', touchStart, {
      passive: false,
      capture: false,
    });
    ...
    // Přidat událostový posluchač na tlačítko "closeBtn"
    addEvent(
      closeBtn,
      'click',
      function (e) {
    ...
    self.showGallery = function showGallery() {
      if (activeInstance !== self) {
        ...
        if (caption && items.length)
          caption.textContent = String(index + 1) + '/' + String(items.length);
        showOverlay(self);
    ...
    self.hideGallery = function hideGallery() {
      if (activeInstance === self) {
        removeExtraHandlers();
        removeEvent(window, 'keydown', keyPress, {
          passive: false,
          capture: false,
        });
        hideOverlay();
    ...
    self.showNext = function showNext() {
      ...
      if (index + 1 < items.length) {
        ...
        setTimeout(function () {
          removeClass(slider, 'tt-right-spring');
        }, 500);
      }
      return self;
    };
    ...
    self.showPrevious = function showPrevious() {
      ...
      if (index > 0) {
        ...
        setTimeout(function () {
          removeClass(slider, 'tt-left-spring');
        }, 500);
      }
      return self;
    };
    ...
    self.dispose = function () {
      if (slider) {
        ...
        removeEvent(slider, 'touchstart', touchStart, {
          passive: false,
          capture: false,
        });
        removeEvent(slider, 'mousedown', touchStart, {
          passive: false,
          capture: false,
        });
        removeEvent(slider, 'wheel', wheelTurn, {
          passive: false,
          capture: false,
        });
        removeEvent(prevArrow, 'click', prevClick, {
          passive: false,
          capture: false,
        });
        removeEvent(nextArrow, 'click', nextClick, {
          passive: false,
          capture: false,
        });
        if (itemClick)
          items.forEach(function (item, i) {
            removeEvent(item, 'click', itemClick[i], {
              passive: false,
              capture: false,
            });
          });
